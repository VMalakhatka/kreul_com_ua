/*
  Paint_Rus ONLY. Manual upgrade in SQL Query Analyzer.

  Candidate: deterministic receipt/return arithmetic (2026-09-07).
  Does NOT change the legacy old SUM_UCHET selection policy.
  See 36_negative_correction_validation.md for rollout gates and limitations.

  Purpose:
    - upgrades an existing dbo.LAVKA_I_UCHET_1_TOVAR_SAFE;
    - keeps average accounting with N_4 IS NULL;
    - accepts both uch_nal=0 (N_2=1000) and uch_nal=1 (N_2=1100);
    - preserves the existing dbo.LAVKA_I_UCHET_TOVAR_SAFE wrapper;
    - never alters the original Folio dbo.I_UCHET_* procedures;
    - ALTER PROCEDURE preserves existing EXECUTE permissions.

  Run without an open transaction. After success, execute rollback-preview only.
*/
SET NOEXEC OFF
GO
IF DB_NAME() <> 'Paint_Rus'
BEGIN
    RAISERROR('STOP: this script may run only in Paint_Rus', 16, 1)
    SET NOEXEC ON
END
...
BEGIN
    RAISERROR('STOP: compatibility level must be 80', 16, 1)
    SET NOEXEC ON
END
...
IF OBJECT_ID('dbo.I_UCHET_1_TOVAR') IS NULL
   OR OBJECT_ID('dbo.I_UCHET_TOVAR') IS NULL
   OR OBJECT_ID('dbo.LAVKA_I_UCHET_1_TOVAR_SAFE') IS NULL
   OR OBJECT_ID('dbo.LAVKA_I_UCHET_TOVAR_SAFE') IS NULL
BEGIN
    RAISERROR('STOP: required Folio or LAVKA procedures are missing', 16, 1)
    SET NOEXEC ON
END
...
BEGIN
    RAISERROR('STOP: unexpected LAVKA procedure parameter contract', 16, 1)
    SET NOEXEC ON
END
GO

ALTER PROCEDURE dbo.LAVKA_I_UCHET_1_TOVAR_SAFE @n_group int, @id_sclad int, @usredn bit=0,
...
DECLARE @ret int, @artic varchar(20), @time0 datetime, @lavka_denominator float,
        @lavka_numerator float, @lavka_currency_numerator float
...
BEGIN
  ...
  BEGIN
	...
	WHERE /*CURRENT OF move*/ RECNO=@recno
     ...
     END
      ...
      BEGIN
         ...
         END
         -- Freeze operands before assigning running balances. Do not rely on
         -- SELECT variable-assignment order (SQL Server 2000).
	...
	WHERE /*CURRENT OF move*/ RECNO=@recno
      ...
      BEGIN
         SELECT @lavka_denominator=ISNULL(@kol0,0)+ISNULL(@kolc_predm,0),
                @lavka_numerator=@sum0+@sum,
                @lavka_currency_numerator=@sum0v+@sumv
         IF ABS(@lavka_denominator)<=0.00000000001
         BEGIN
           SELECT @problem_code='ZERO_ACCOUNTING_DENOMINATOR',
                  @problem_art=@artic, @problem_recno=@recno,
...
SELECT DB_NAME() AS database_name,
       ...
       (SELECT COUNT(*)
          FROM dbo.syscolumns
         WHERE id=OBJECT_ID('dbo.LAVKA_I_UCHET_1_TOVAR_SAFE')) AS parameter_count,
       'AVERAGE_OPERANDS_FROZEN_V1' AS upgrade_status
GO

-- Expected: current database, non-NULL procedure id, parameter_count=20.
-- No GRANT is required again: ALTER PROCEDURE preserves permissions.
SET NOEXEC OFF
GO
