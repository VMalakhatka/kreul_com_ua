/*
  Paint_Ua ONLY. Manual installation in SQL Query Analyzer.

  Validated source:
    - SQL Server 8.00 / compatibility level 80;
    - the executable definitions of Paint_Ua I_UCHET_1_TOVAR and
      I_UCHET_TOVAR have the same semantic SHA-256 as the Paint_Rus objects;
    - the LAVKA procedures passed rollback and commit golden-master tests in
      the isolated Paint_Rus database.

  Safety properties:
    - creates separate dbo.LAVKA_* procedures;
    - never alters dbo.I_UCHET_*;
  ...
  IMPORTANT: SET NOEXEC is used so a failed guard stops all following GO
  batches. If a guard fails, close this Query Analyzer window before doing
  anything else.
*/
SET NOEXEC OFF
GO
...
IF @@TRANCOUNT <> 0
BEGIN
    RAISERROR('STOP: install must start without an open transaction', 16, 1)
    SET NOEXEC ON
END
...
BEGIN
    RAISERROR('STOP: required Folio I_UCHET_* procedures are missing', 16, 1)
    SET NOEXEC ON
END

IF OBJECT_ID('dbo.LAVKA_I_UCHET_1_TOVAR_SAFE') IS NOT NULL
   OR OBJECT_ID('dbo.LAVKA_I_UCHET_TOVAR_SAFE') IS NOT NULL
BEGIN
    RAISERROR('STOP: LAVKA safe procedures already exist; this first-install script will not overwrite them', 16, 1)
    SET NOEXEC ON
END
GO
...
BEGIN
    ...
    RAISERROR('INSTALL FAILED: one or both LAVKA procedures are missing', 16, 1)
    SET NOEXEC ON
END
ELSE
BEGIN
...
SELECT DB_NAME() AS database_name,
       ...
       (SELECT cmptlevel
          FROM master.dbo.sysdatabases
         WHERE name = DB_NAME()) AS compatibility_level,
