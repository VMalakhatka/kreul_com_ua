...
Для `NATIVE_SAFE_PROCEDURE_UNSUPPORTED_SCOPE_OR_MODE` (return code `32`)
кампания завершается как ошибка склада: «Зупинено: несумісна версія процедури
ФОЛІО». Это не `COMPLETED_WITH_WARNINGS`, не пропущенный SKU и не основание
автоматически повторять пакет. Для `N_2=1100` оператор обновляет
safe-процедуру, затем повторяет только preview. Код `31` означает отсутствие
обязательной внешней транзакции; неизвестные коды отображаются как
`NATIVE_SAFE_PROCEDURE_UNDOCUMENTED_CODE` и также останавливают склад.
