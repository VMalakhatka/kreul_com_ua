...
Важно: в текущей базе ФОЛИО `SCL_NAKL.N_PLAT_POR` имеет тип `float NOT NULL`, поэтому `documentNumber` должен быть числовым. Номер вида `WEB-2026-00001` нельзя писать в это поле напрямую; для внешнего текстового номера нужно отдельное подтверждённое поле или интеграционная таблица.
...
- если `folio_account_header.documentNumber` пустой, Java берёт следующий видимый номер из штатного счётчика ФОЛИО `NSF_ORG.N_3` и записывает его в `SCL_NAKL.N_PLAT_POR`.
...
Нумерация `document_number`:

- штатный счётчик ФОЛИО: `NSF_ORG.N_3`;
- используется общая нумерация по всем складам: `NSF_ORG.ID_SCLAD = 0`;
- обычный учитываемый счёт использует строку счётчика с последними символами `NAME_USER = СU`;
- неучитываемый счёт, включая `missing_stock_account`, использует строку счётчика с последними символами `NAME_USER = СN`;
- сравнение делается как `RIGHT(NAME_USER, 2)`, потому что в базе перед кодом хранится специальный пробельный байт `0xA0`;
- `С` — кириллическая буква, `U` / `N` — латиница.
...
- `documentNumber` должен быть числовым, потому что в ФОЛИО это `SCL_NAKL.N_PLAT_POR float`. Для Woo можно передавать пустую строку: Java возьмёт номер из `NSF_ORG.N_3`.
