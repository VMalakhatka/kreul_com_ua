...
warehouses и SKU текущей страницы, generation ACTIVE/schema 7.

По уточнённому правилу владельца от 2026-09-20 потребитель прибавляет всю сумму
этих резервов к свободному остатку **sourceWarehouseId**, независимо от получателя.
Группа суммирует исправленные остатки своих складов. `sourceInfo` служит только
для показа документа: маршруты, распределение по получателям и дополнительное
подтверждение networkSnapshotConsistency для этой поправки не используются.
Клиентские резервы остаются вычтенными; regularSoldUnits/stockoutDemand не меняются.
Потребитель проверяет поколение того же склада и сумму internal <= physical−available.
Существующий отдельный контракт физического транзита не меняется. DTO и schema 7
совместимы с уточнённым правилом: повторная миграция или пересборка v7 не требуются.
