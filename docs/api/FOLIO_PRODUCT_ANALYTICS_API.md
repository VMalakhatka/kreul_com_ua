...
гарантированным товаром в пути. `suppliers` содержит контрагентов подтверждённых
...
| `MIN_TVRZAP = 1` | `FORECAST_ONLY` | заказ определяется прогнозом, дополнительный резерв равен 0 |
| `MIN_TVRZAP > 1` | `FORECAST_PLUS_MINIMUM_STOCK` | к прогнозной потребности добавить `reserveAboveForecast = MIN_TVRZAP` |
