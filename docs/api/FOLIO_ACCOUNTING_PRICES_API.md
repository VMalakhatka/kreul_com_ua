...
Предварительный native-скан может вернуть четыре кода безопасного пропуска:
...
| `ZERO_ACCOUNTING_PRICE_WITH_SALE_PRICE` | у пустой карточки нет учитываемых движений, начальный остаток и применимая учётная цена равны нулю, но задана продажная цена и включён автоматический расчёт наценки |

У всех четырёх `details.skipped=true`: этот SKU не изменяется, но обработка других
товаров продолжается.

Для `ZERO_ACCOUNTING_PRICE_WITH_SALE_PRICE` в `details` передаются `sku`,
`warehouseId`, `movementCount=0`, `initialQuantity`, рублёвая и валютная
учётные/продажные цены, `priceBasis`, `fixedMarkup`, `currentState`,
`source=JAVA_ZERO_PRICE_PREFLIGHT` и `skipped=true`. Это консервативная защита
по результату рабочего rollback-preview: SQL exception не возвращает точный
внутренний SKU порции, поэтому API не выдаёт checkpoint за доказанный виновный
товар.
...
причины и диагностикой. Для хронологических причин в нём есть документ и
количество до/после операции; для `ZERO_ACCOUNTING_PRICE_WITH_SALE_PRICE` —
учётные и продажные цены, режим цены и `movementCount=0`. Неожиданная остановка
