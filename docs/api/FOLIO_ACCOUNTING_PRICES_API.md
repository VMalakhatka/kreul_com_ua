...
Подтверждены два кода **средней учётной цены**: `SCLAD_R.N_2=1000`
(`includeTax=false`) и `SCLAD_R.N_2=1100` (`includeTax=true`), при
`SCLAD_R.N_4 IS NULL`. Режим `1100` прошёл rollback golden-master штатной
`I_UCHET_TOVAR` на Paint_Rus. До установки обновлённой
`LAVKA_I_UCHET_*_SAFE` в конкретную базу native route для `1100` должен
считаться неразвёрнутым: старый safe-объект вернёт защитный код 32 и транзакция
будет откатана.
...
На текущем этапе rollback-preview разрешён для `Paint_Rus` и `Paint_Ua` для
средней цены `SCLAD_R.N_2=1000/1100` и `SCLAD_R.N_4 IS NULL`, если в целевой
базе установлена версия `LAVKA_I_UCHET_*_SAFE`, принимающая `uch_nal=0/1`.
...
Если во время длинной порции менеджер или интеграция создаёт/изменяет документ
и защищённое состояние одного из выбранных SKU меняется уже после его
индивидуального postcheck, Java не останавливает всю порцию. Финальная проверка
сравнивает состояния по SKU:

- неизменившиеся committed SKU получают fingerprint и подтверждаются в
  MariaDB как обычно;
- изменившийся SKU не получает ложное подтверждение и возвращается в warning
  `SELECTED_PRODUCT_CHANGED_DURING_JOB`;
- warning содержит `sku`, `warehouseId`, `changeKind`, признаки изменения
  карточки/движений, количество движений до/после,
  `recalculationCommitted`, `verificationRecorded=false` и рекомендацию;
- job завершается `COMPLETED_WITH_WARNINGS`, поэтому оркестратор продолжает
  следующие товары и склады;
- обязательный финальный snapshot повторно классифицирует изменившийся товар,
  и следующая кампания выберет его, если он остаётся `DIRTY`.

Это восстановление применяется только к доказанному конкурентному изменению
после успешных отдельных commit. Неизвестная SQL/contract/transaction ошибка
по-прежнему возвращает `FAILED`/`FAILED_PARTIAL`, а недоказанный результат
commit — `OUTCOME_UNKNOWN`; такие результаты автоматически не повторяются.
...
| `procedureTotalUnits` | сырой `n_tot` процедуры; может быть `0` |

Нулевой `procedureTotalUnits` не является ошибкой, если Java знает ненулевой
`totalUnits`, процедура вернула допустимый код, обработанный артикул совпадает с
запрошенным, курсор корректен в CP1251-collation, транзакционная граница не
изменилась и защищённые инварианты прошли postcheck. При неизвестном размере
кампании возвращается `NATIVE_RANGE_TOTAL_UNKNOWN`, а при реальном нарушении
OUT-контракта — `NATIVE_RANGE_CONTRACT_INVALID`.

В `failedChunk` поля `procedureCurrentUnits`/`procedureTotalUnits` всегда
...
- разрешены экспериментально подтверждённые коды средней цены:
  `SCLAD_R.N_2=1000` (`calculationMode=0`, `periodMode=0`, без налога) и
  `SCLAD_R.N_2=1100` (тот же алгоритм, `includeTax=true`);
