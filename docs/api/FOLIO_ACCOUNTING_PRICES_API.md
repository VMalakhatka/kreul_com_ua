...
3. пропускает заранее диагностированные проблемные SKU и проверяет весь
   остальной склад;
4. если rollback-проход получает `Divide by zero`, Java откатывает порцию,
   по фактическому CP1251-порядку повторяет только rollback-пробы и находит
   минимальный подтверждённый непрерывный диапазон; это один SKU, когда ошибка
   воспроизводится независимо, либо несколько соседних SKU, когда арифметическое
   состояние legacy-процедуры зависит от последовательности карточек;
5. подтверждённый диапазон добавляется в quarantine, после чего исходная
   порция повторяется; глобальные арифметические настройки MSSQL не подавляются;
6. если процедура обнаружила новую, не распознанную предварительной диагностикой
   проблему `otr_date`, возвращает `BLOCKED_NEGATIVE_STOCK`, не фиксируя ни одной порции;
7. успешный preflight запускает второй проход с commit каждой успешной порции,
...
| Поле | Значение |
|---|---|
| `phase` | стадия выполнения: `QUEUED`, `DIAGNOSTIC_SCAN`, `DIVIDE_BY_ZERO_ISOLATION`, `QUARANTINE_PREPARATION`, `PRECHECK_RUNNING`, `PRECHECK_COMPLETED`, `APPLY_RUNNING`, `APPLY_COMPLETED`, `APPLY_STOPPED` или `FAILED` |
...
от входного `inputArt` к продолжению `nextArt`. Поле `outputArt` является сырым
OUT-значением старой процедуры и используется только для диагностики: живой
эксперимент показал, что оно не всегда является последним товаром порции.
Для apply конец защищённого диапазона вычисляется в MS SQL как артикул,
предшествующий `nextArt`, либо как максимальный артикул склада при завершении.
Фронт не должен самостоятельно сортировать или валидировать эти поля.
...
Если деление на ноль проявилось только внутри legacy-процедуры и не было
предсказано сканером, rollback-preview дополнительно возвращает:

| Код | Подтверждение |
|---|---|
| `ACCOUNTING_PRICE_DIVIDE_BY_ZERO` | `I_UCHET_TOVAR` повторно дала ту же арифметическую ошибку на подтверждённом непрерывном диапазоне из одного или нескольких SKU |

У warning всегда есть `rangeConfirmed=true`, `firstSku`, `lastSku`, `skuCount`,
`skus[]`, `skusTruncated` и число isolation-проб. Для независимой ошибки одного
товара дополнительно возвращаются `exactSkuConfirmed=true`,
`source=FOLIO_SINGLE_SKU_ROLLBACK_PROBE` и доступное текущее состояние карточки.
Если один SKU чист, но связка соседних карточек воспроизводит ошибку, Java
возвращает `exactSkuConfirmed=false` и
`source=FOLIO_MULTI_SKU_ROLLBACK_PROBE`; весь подтверждённый диапазон временно
пропускается. `checkpointArt` остаётся только входным курсором оборвавшейся
time-based порции и не объявляется виновником. Во время apply новый, не найденный
preflight делитель не обходится:
текущая порция откатывается и job останавливается.

После SQL error 8134 Java не пытается восстанавливать временный quarantine
дополнительными SQL-командами на том же соединении: такая компенсация может
скрыть исходную арифметическую ошибку, если legacy-транзакция уже повреждена.
Вся порция передаётся Spring transaction manager для rollback. Явное
восстановление `TIP_TOVR` выполняется только после успешного возврата процедуры,
когда транзакция ещё может быть зафиксирована.
...
- `UPDATE` на `dbo.SCL_ARTC` — временно назначить и вернуть тип
  диагностированным SKU; во время rollback-изоляции деления на ноль также
  временно скрыть товары вне проверяемого диапазона. Все isolation-транзакции
  обязательно откатываются;
