...
4. если rollback-проход получает `Divide by zero`, Java откатывает порцию,
   двоичным поиском по фактическому CP1251-порядку повторяет только rollback-пробы,
   подтверждает ошибку на одном SKU, добавляет его в quarantine и повторяет
   исходную порцию; глобальные арифметические настройки MSSQL не подавляются;
5. если процедура обнаружила новую, не распознанную предварительной диагностикой
   проблему `otr_date`, возвращает `BLOCKED_NEGATIVE_STOCK`, не фиксируя ни одной порции;
6. успешный preflight запускает второй проход с commit каждой успешной порции,
...
| Код | Подтверждение |
|---|---|
| `ACCOUNTING_PRICE_DIVIDE_BY_ZERO` | `I_UCHET_TOVAR` повторно дала ту же арифметическую ошибку, когда в откатываемой пробе был видим ровно один SKU |

У warning есть `details.sku`, `exactSkuConfirmed=true`,
`source=FOLIO_SINGLE_SKU_ROLLBACK_PROBE`, `checkpointArt`, число isolation-проб
и доступное текущее состояние карточки. Это точный виновный SKU, а не входной
checkpoint оборвавшейся time-based порции. После подтверждения SKU временно
скрывается только на время последующих транзакций, а проверка остальных товаров
продолжается. Во время apply новый, не найденный preflight делитель не обходится:
текущая порция откатывается и job останавливается.
