...
### Восстановление после остановки Java (2026-09-24)

Lease по умолчанию истекает через 120 секунд после последнего подтверждения жизни
процесса. Отдельный daemon heartbeat продлевает её каждые 15 секунд, независимо от
длительности запроса ФОЛИО и числа рассчитанных клиентов. `lastHeartbeatAt` в building
по-прежнему отражает прогресс, а не отдельный lease heartbeat. Страница при очередном
status-запросе видит `running=false` после истечения lease и доступность повторного
запуска; watchdog проверяет recovery каждые 30 секунд. Пока lease действительна,
нельзя снимать её только из-за отсутствия SQL-сессии. Старая версия/явный override
`LAVKA_FOLIO_BALANCE_SNAPSHOT_LEASE_SECONDS=7200` сохраняет двухчасовое ожидание;
уже выданная старая lease не сокращается задним числом при обновлении Java.
