...
## Список после просмотра индивидуального баланса

После успешного `GET /admin/folio/customer-balance` по всем складам с
`includeServicePayments=true` (значение по умолчанию) Java точечно освежает
канонический баланс клиента **независимо от `dateFrom`**. Для месячного отчёта
выполняется дополнительное чтение полной истории этого клиента; месячная
детализация в ответе не меняется. Новый запрос/endpoint от фронта не нужен.

При возвращении в список повторить `GET /admin/folio/customer-debtors` с текущими
фильтрами; перерисовать строки, итоги и пагинацию. Если последняя страница стала
пустой после исключения погашенного долга, скорректировать offset и запросить её
снова. При `minPayable=0` показываются только положительные `payableNow`.
Не заменять строку локально всем `summary` месячного отчёта: ПРД/отсрочки зависят
от периода. Поиск/обновление самого списка не запускает live-перерасчёт ФОЛИО.

Обрабатывать дополнительные `warnings` индивидуального ответа:

- `BALANCE_SNAPSHOT_CLIENT_REFRESH_FAILED`: детализация получена, но каноническое
  чтение или запись снимка не удались; показать предупреждение, предложить повтор
  индивидуального отчёта или штатное обновление снимка. Не писать «список обновлён».
- `BALANCE_SNAPSHOT_CLIENT_REFRESH_DEFERRED`: результат сохранён в live override,
  но подходящая строка не обновлена. Проверить status: возможно, готовый снимок
  относится к предыдущему дню или клиента ещё нет в нём. Предложить полный refresh,
  не запускать его автоматически на каждом открытии страницы.

Отчёт по части складов либо без служебных оплат общий снимок не меняет.
Строки клиентов, которых не открывали, обновляются полным плановым/ручным refresh.
`completedAt` поколения не означает единую свежесть всех клиентов: строки
рассчитываются последовательно, а live-запросы затем обновляют отдельные строки.
Поля JSON списка не изменились. Точный контракт, ограничения NOLOCK и защита
публикации описаны в [API — обновление одного клиента](FOLIO_CUSTOMER_BALANCE_API.md#обновление-при-запросе-одного-клиента).
