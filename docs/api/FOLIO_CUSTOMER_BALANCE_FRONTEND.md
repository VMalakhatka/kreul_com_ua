...
`commonDebt` равен сальдо ФОЛИО: все входящие оплаты, включая `ПРД`, уменьшают долг. `prepaymentAmount` — справочная сумма отмеченных платежей за выбранный период, а не остаток неиспользованного аванса; повторно к долгу не прибавляется.
...
- прибавлять справочную сумму `ПРД` обратно к долгу или считать её неиспользованным авансом.

В старом Excel была формула только по кассе. API намеренно исправляет эту ошибку и учитывает `bankPayment + cashPayment`.

Удаление `ПРД` из старых платежей для исправления веб-отчёта не требуется. Маркер классифицирует платёж, но не отменяет полученную оплату.
...
### Исправление исторических предоплат — 2026-09-24

Старый расчёт прибавлял все `ПРД` в выбранном периоде обратно к сальдо;
перенос того же платежа во входящий остаток менял результат. Общий калькулятор
теперь сохраняет бухгалтерское сальдо. Классификация `РЕЛ` не изменена.
Регрессия проверяет одинаковый долг 3449.29 для полной истории и месячного
отчёта при исторических отмеченных оплатах 169892.00. Тест использует синтетические
строки, без доступа к живой ФОЛИО.

После deploy Java ранее сохранённый снимок остаётся со старой формулой:
необходимо отдельно запустить штатное обновление снимка и дождаться READY.
До этого сводный отчёт не считается исправленным на production. PHP не должен
сам вычитать предоплату из старого ответа: это даст двойное вычитание после refresh.
