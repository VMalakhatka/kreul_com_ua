...
```json
{
  ...
  "rows": [
    ...
    },
    {
      ...
      "cashPayment": 0.00,
      "balanceAfter": 900.00,
      "note": "222 Банковская предоплата",
      "invoiceDate": null,
      "openingBalanceRow": false,
      "deferred": false,
      "overdueDeferred": false,
...
| `deferredAmount` | На отсрочке/реализации | накладные с маркером `111`, срок оплаты которых ещё не наступил |
| `overdueDeferredAmount` | Просрочено по отсрочке/реализации | документы с `111`, срок которых уже наступил |
| `prepaymentAmount` | Предоплата | банк **плюс** касса по платежам с примечанием `222...` |
...
Почему предоплата прибавляется в `payableNow`: входящий платёж уже уменьшил `commonDebt`, но платёж с `222` предназначен под заказанный товар и не должен погашать текущий долг. Поэтому backend исключает его влияние из суммы «к оплате».

`overdueDeferredAmount` уже входит в `commonDebt` и `payableNow`. Его нельзя ещё раз прибавлять к сумме «к оплате».

## 8. Предоплата `222`

Предоплатой является **любой платёжный документ**, примечание которого начинается с `222`:

```text
222Предоплата
222 Банковская предоплата
```
...
- повторно искать `222` и самостоятельно пересчитывать сумму на frontend;
...
|---|---|---|
| `basis` начинается с `111` и срок ещё не наступил | `deferred=true` | светло-жёлтая строка; явно показать `controlDate` |
| срок наступил, `111` остался | `overdueDeferred=true` | красное предупреждение; `basis` выделить красным |
...
Одна будущая `controlDate` без маркера `111` в `basis` не является отсрочкой. В обычных накладных контрольная дата не является контролируемым бизнес-признаком. Backend обязан вернуть для такой строки `deferred=false`; frontend не должен исправлять это своей формулой, но должен считать ответ с `deferred=true` дефектом API.
...
- `prepayment=true` — зелёный фон/маркер; в `note` визуально выделить начало `222`.
...
10. Будущая контрольная дата без `basis`, начинающегося с `111`, не попадает в отсрочку.
