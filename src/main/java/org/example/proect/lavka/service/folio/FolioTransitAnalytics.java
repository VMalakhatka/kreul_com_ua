...
public final class FolioTransitAnalytics {
    ...
    public static TransitCapability capability(TransitCalculation config,
        ...
        return new TransitCapability(single == null ? null : single.warehouseId(),
                ...
                "WAREHOUSE_BALANCE_WITH_CONFIRMED_SUPPLIER_ORIGIN", true, VERSION, MAX_WAREHOUSES,
    ...
    public static InTransitStock stock(TransitCapability capability,
        ...
        boolean ready = capability.ready() && sources.stream().allMatch(s -> s.availableForPlanningQuantity() != null);
        ...
        String status = !capability.enabled() ? "DISABLED"
                ...
                : !ready ? single == null ? "INCOMPLETE_TRANSIT_DATA" : single.status()
                : known.signum() == 0 ? "NO_IN_TRANSIT_STOCK" : "CONFIRMED_SUPPLIER_ORIGIN";
        ...
        return new InTransitStock(single == null ? null : single.warehouseId(),
                ...
                status, capability.enabled() ? ready : null,
                sum(sources, TransitSourceStock::physicalQuantity), sum(sources, TransitSourceStock::reservedQuantity),
                sum(sources, TransitSourceStock::availableQuantity), ready ? known : null,
                ...
                known, sources, warnings.stream().distinct().toList());
    }
    ...
    private static TransitSourceStock source(TransitSourceCapability cap, TransitRow row) {
        ...
        if (status == null) {
            if (row.physicalQuantity() == null || row.reservedQuantity() == null || row.availableQuantity() == null
                    || row.openingQuantity() == null) status = "INCOMPLETE_TRANSIT_DATA";
            ...
            else if (row.physicalQuantity().signum() == 0) { status = "NO_IN_TRANSIT_STOCK"; planning = BigDecimal.ZERO; }
            else if (row.openingQuantity().signum() != 0) status = "OPENING_BALANCE_UNATTRIBUTED";
            else if (row.inboundCount() == 0) status = "NO_CONFIRMED_INBOUND";
            else if (row.supplierInboundCount() != row.inboundCount()) status = "MIXED_ORIGIN";
            else { status = "CONFIRMED_SUPPLIER_ORIGIN"; planning = row.availableQuantity(); }
        }
        ...
        return new TransitSourceStock(cap.warehouseId(), cap.warehouseName(), cap.generationId(), status,
                ...
                planning == null ? List.of(status) : List.of());
    }
