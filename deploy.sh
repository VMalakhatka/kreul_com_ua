...
# ---- Базовые переменные (дефолты, можно переопределить извне) ----
: "${IMAGE_NAME:=kreul-api:latest}"
: "${CONTAINER_NAME:=kreul-api}"
: "${ENV_LOCAL_FILE:=.env.prod}"
: "${REMOTE_HOST:=kreul}"
: "${REMOTE_DIR:=~/kreul-api}"
...
ssh -t "$REMOTE_HOST" bash -lc "'
  ...
  docker run -d --name $CONTAINER_NAME \
    ...
    $IMAGE_NAME
  echo \"→ Ждём health: $INTERNAL_HEALTH…\"
  for i in {1..20}; do
...
echo -e "\n✅ Готово. Логи:  ssh $REMOTE_HOST 'docker logs -f $CONTAINER_NAME'"