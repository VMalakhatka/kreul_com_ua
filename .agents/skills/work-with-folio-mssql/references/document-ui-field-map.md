...
## Месячный мастер-класс Одессы

Для отчёта прибыли подтверждена отдельная read-only выборка строк мастер-класса:

- заголовок накладной — `SCL_NAKL`, строка движения — `SCL_MOVE`, связь по
  `UNICUM_NUM`;
- склад — точный `SCL_MOVE.ID_SCLAD=5`;
- месяц — `SCL_NAKL.DATE_P_POR >= monthStart AND < nextMonthStart`;
- артикул строки — точный `SCL_MOVE.NAME_PREDM`; существование артикула
  проверяется по `ALL_ARTC.COD_ARTIC`;
- уникальная строка — `SCL_MOVE.RECNO`;
- номер, дополнительный номер и номер строки — `SCL_NAKL.N_PLAT_POR`,
  `SCL_NAKL.DOPN_SCHET`, `SCL_MOVE.NUM_PREDMT`;
- вид операции — `SCL_NAKL.VID_DOC`, при пустом значении используется
  `SCL_MOVE.VID_DOC`;
- сумма строки — `SCL_MOVE.SUM_PREDM`, учётная себестоимость —
  `SCL_MOVE.SUM_UCHET`;
- доход: проведённая расходная строка `Р` без признаков возврата;
- возврат: проведённая приходная строка `П` с видом операции `ВОЗВРАТ`.

Точные месячные артикулы существуют в исследованной схеме Paint_Rus. Сентябрь
исторически записан как `Мастер-Класс сентябр` без мягкого знака. Контрольные
списки владельца для июля 2026 содержат 17 доходных строк на 10 740 грн и 9
возвратных строк на 6 050 грн. Это подтверждение интерфейсом владельца, а не
прямая SQL-сверка Paint_Ua; после деплоя обязательно сверить `/audit` с этими
документами.
