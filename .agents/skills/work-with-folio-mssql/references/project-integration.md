...
Подтверждённая карта штрихкода: основной GTIN находится в
`SCL_ARTC.DOP3_ARTIC`; `CardTovExportDaoImpl` публикует его как
`global_unique_id`, а Woo-синхронизация сохраняет в `_wc_gtin_code`. Таблица
`SCL_CODE` содержит дополнительные штрихкоды и не должна подменять основной
GTIN без отдельного правила выбора.

Для аналитического товара в пути используется склад 9 «Транспорт» и уже
опубликованные movement facts. Поставщицкое происхождение подтверждается только
для `PURCHASE_RECEIPT` от организаций типов `Т`/`I`, при отсутствии
необъяснённого положительного opening balance и другого положительного
входящего потока. Смешанное происхождение нельзя автоматически включать в
закупочную доступность. Поставщик карточки, контрагент прихода и менеджер продаж
— три разные сущности; неподтверждённый sales manager не называть seller.
...
дают net-прибыль. Подтверждённый доступный остаток склада 9 уже можно учитывать
как товар в пути, однако до добавления lead time, MOQ, кратности и открытых
заказов разрешено показывать дефицит/покрытие, но не готовое количество закупки.
